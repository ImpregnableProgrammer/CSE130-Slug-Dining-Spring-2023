...
// Tests for extra credit, where cleanber thread is spawned before student threads are and after one other is, so the cleaner should enter once the first student leaves rather than the other two students
int main(void) {
  ...
  msleep(100); // Sleeps are used to enforce thread order
